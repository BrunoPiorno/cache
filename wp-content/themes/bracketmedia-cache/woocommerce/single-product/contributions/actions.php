...
?>
<?php 
if($contribution->type == 'contribution_comment') {
	$contribution->type = '';
}
?>
<p class="contribution-actions">
	
	Was this <?php echo $contribution->type; ?> helpful? 
	<a href="javascript:void(0);" class="vote vote-up js-tip <?php if ( 'positive' == $contribution->get_user_vote() ) : ?>done<?php endif; ?>" rel="nofollow" data-comment-positive="<?php echo absint( $contribution->get_positive_votes() ); ?>" data-comment-id="<?php echo esc_attr( $contribution->get_id() ); ?>" title="<?php esc_attr_e( 'Upvote if this was helpful', 'woocommerce-product-reviews-pro' ); ?>"><i class="fas fa-thumbs-up"></i></a>
	<span class="vote-count vote-count-positive">
		(<span><?php echo absint( $contribution->get_positive_votes() ); ?></span>)
	</span>
	<a href="javascript:void(0);" class="vote vote-down js-tip <?php if ( 'negative' == $contribution->get_user_vote() ) : ?>done<?php endif; ?>" rel="nofollow" data-comment-positive="<?php echo absint( $contribution->get_negative_votes() ); ?>" data-comment-id="<?php echo esc_attr( $contribution->get_id() ); ?>" title="<?php esc_attr_e( 'Downvote if this was not helpful', 'woocommerce-product-reviews-pro' ); ?>"><i class="fas fa-thumbs-down"></i></a>
